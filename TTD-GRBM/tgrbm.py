...
import numpy as np
import helpers_tgrbm as helpers


class tgrbm():
    
    def __init__(self, sigma=1.0, hid_shape=[20,25], r=[2,5,5,5,2], maxepoch=5,
                 batchsize=100, K=1, momentum=0.7, learning_rate=0.01,                 
                 lambda_1=0.0, lambda_2=0.000, verbose=True):
        ''' This function initializes the TTGRBM model learning algorithm. 
    ...
    def gradients(self, data):
        ...
        for l in range(self.l_shape):
            grad_cores['gradG' + str(l)] = tf.math.subtract( first_cores_term['gradE_G'+str(l)],\
                                                            second_cores_term['gradE_G' + str(l)]) - self.lambda_1*self.cores['G'+str(l)] - self.lambda_2*tf.math.sign(self.cores['G'+str(l)])
    ...
    def optimize_mpo_grbm(self, dataset):
        ...
        for epoch in range(self.maxepoch):
            ...
            for batch in range(numbatches):
                ...
                self.gradients(data)
                self.update_params_mpo(self.gradient_visbiases,
                                       self.gradient_hidbiases, self.gradient_cores)
